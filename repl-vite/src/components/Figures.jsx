...
function renderAxes(svg,ax,ox,oy,availW,availH){
  ...
  if(cfg.polar){
    ...
    (ax.datasets||[]).forEach((ds,idx)=>{
      ...
      if(hasLine&&points.length>1){const thetaRange=ds.x.length>1?Math.abs(ds.x[ds.x.length-1]-ds.x[0]):0;const curve=thetaRange>=Math.PI*1.9?d3.curveLinearClosed:d3.curveLinear;dataG.append("path").datum(points).attr("d",d3.line().x(d=>d[0]).y(d=>d[1]).curve(curve)).attr("fill","none").attr("stroke",color).attr("stroke-width",lw).attr("stroke-dasharray",dasharray);}
  ...
  if(axisMode==='equal'){
    ...
    const dataAspect=xRange/yRange,viewAspect=iw/ih;
    if(dataAspect>viewAspect){const ny=xRange*ih/iw;const ym=(yMin+yMax)/2;yMin=ym-ny/2;yMax=ym+ny/2;}
    else{const nx=yRange*iw/ih;const xm=(xMin+xMax)/2;xMin=xm-nx/2;xMax=xm+nx/2;}
...
function FigurePanel({figure,onClose}){
  ...
  // Redraw when figure data OR container width changes
  useEffect(()=>{
    if(!figure||!svgRef.current||containerWidth<10)return;
    const svg=d3.select(svgRef.current);
    svg.selectAll("*").remove();
    const cw=containerWidth-12; // padding
    const axesList=getAxesList(figure);
    const grid=figure.subplotGrid;

    if(grid&&grid.length>=2&&axesList.length>1){
      const[rows,cols]=grid;
      const cellW=Math.max(120,cw/cols);
      const cellH=Math.max(100,Math.min(220,cellW*0.65));
      const totalH=cellH*rows+8;
      svg.attr("width",cw).attr("height",totalH);
      svg.append("defs");
      axesList.forEach(ax=>{
        const p=ax.subplotIndex||1;
        const row=Math.floor((p-1)/cols);
        const col=(p-1)%cols;
        renderAxes(svg,ax,col*cellW+4,row*cellH+4,cellW-8,cellH-8);
      });
    }else{
    ...
    <div ref={containerRef} style={{background:C.bg2,border:`1px solid ${C.border}`,borderRadius:6,padding:6,position:"relative"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4,padding:"0 2px"}}>
        <span style={{fontSize:9,color:C.textMuted}}>
          Figure {figure.id}{grid&&` · subplot(${grid[0]}×${grid[1]})`}
        </span>
...
export default function Figures({figures,onSetFigures,onCloseFigure,onCloseAll,onClose}){
  ...
  for(let i=figures.length-1;i>=0;i--){const id=figures[i].id;if(!seen.has(id)){seen.add(id);deduped.unshift(figures[i]);}}
  return(
