...
const Value *DebugWorkspace::get(const std::string &name) const
{
    auto it = framePtrs_.find(name);
    if (it != framePtrs_.end()) {
        const Value *p = it->second;
...
void DebugWorkspace::set(const std::string &name, const Value &val)
{
