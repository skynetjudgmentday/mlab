...
TEST_P(IntegerTypesTest, Uint8Scalar)
{
    eval("x = uint8(200);");
    auto *x = getVarPtr("x");
    EXPECT_EQ(x->type(), mlab::MType::UINT8);
...
TEST_P(IntegerTypesTest, Int16Array)
{
    ...
    auto *x = getVarPtr("x");
    EXPECT_EQ(x->type(), mlab::MType::INT16);
...
TEST_P(IntegerTypesTest, Uint64Scalar)
{
    ...
    auto *x = getVarPtr("x");
    EXPECT_EQ(x->type(), mlab::MType::UINT64);
}
...
TEST_P(IntegerTypesTest, NoArgConstructor)
{
    ...
    eval("y = single();");
    auto *y = getVarPtr("y");
    EXPECT_EQ(y->type(), mlab::MType::SINGLE);
}
...
TEST_P(IntegerTypesTest, DoubleFromInt)
{
    ...
    auto *x = getVarPtr("x");
    EXPECT_EQ(x->type(), mlab::MType::DOUBLE);
    EXPECT_DOUBLE_EQ(x->toScalar(), 42.0);
}
