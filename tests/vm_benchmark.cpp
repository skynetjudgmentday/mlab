...
public:
    ...
    double runTreeWalker(const std::string &code)
    {
    ...
    BenchResult benchmark(const std::string &name, const std::string &code, int runs = 3)
    {
        ...
        for (int r = 0; r < runs; ++r) {
            ...
            if (vmMs < vmBest) vmBest = vmMs;

            result = vmResult;

            // Verify same result
            EXPECT_NEAR(twResult, vmResult, 1e-6)
                << name << ": TreeWalker and VM results differ";
        }
...
TEST_F(VMBenchmark, Summary)
{
    ...
    auto r1 = benchmark("1. Nested loop 200x200", "s = 0; for i = 1:200; for j = 1:200; s = s + 1; end; end; s;");
    auto r2 = benchmark("2. Scalar math 20000", "x = 0; for i = 1:20000; x = x + i * 0.5 - i / 3.0 + i * i * 0.001; end; x;");
    auto r3 = benchmark("3. Function calls 5000",
        "function r = inc(x); r = x + 1; end; v = 0; for i = 1:5000; v = inc(v); end; v;");
    auto r4 = benchmark("4. Recursive fib(20)",
        "function r = fib(n); if n <= 1; r = n; else; r = fib(n-1) + fib(n-2); end; end; fib(20);");
    auto r5 = benchmark("5. Array index r/w 1000",
        "x = zeros(1, 1000); for i = 1:1000; x(i) = i; end; s = 0; for i = 1:1000; s = s + x(i); end; s;");
    auto r6 = benchmark("6. Branching 20000",
        "c = 0; for i = 1:20000; if mod(i, 3) == 0; c = c + 1; elseif mod(i, 5) == 0; c = c + 2; else; c = c + 3; end; end; c;");
