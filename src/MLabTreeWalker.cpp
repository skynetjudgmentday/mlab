                                ...
                                size_t nargout)
{
    ...
    if (name == "toc") {
        ...
        } else if (engine_.ticCalled_) {
            // toc  →  elapsed since last tic (does NOT consume it)
            start = engine_.ticBase_;
        } else {
        ...
        if (nargout > 0) {
            ...
            result = MValue::scalar(elapsed, &engine_.allocator_);
        } else {
    ...
    if (name == "which") {
        ...
        } else if (engine_.userFuncs_.count(qname)) {
            os << qname << " is a user-defined function.\n";
        } else if (engine_.externalFuncs_.count(qname)) {
            os << "built-in (" << qname << ")\n";
        } else {
            os << "'" << qname << "' not found.\n";
        }
    ...
    if (name == "exist") {
        ...
        if (env->has(varName))
            code = 1;
        else if (engine_.externalFuncs_.count(varName) || engine_.userFuncs_.count(varName))
            code = 5;
        result = MValue::scalar(code, &engine_.allocator_);
        return true;
    }
    ...
    if (name == "class") {
        ...
        result = MValue::fromString(mtypeName(args[0].type()), &engine_.allocator_);
        return true;
    }
