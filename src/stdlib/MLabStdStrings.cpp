...
void StdLibrary::registerStringFunctions(Engine &engine)
{
    engine.registerFunction("num2str",
                            [&engine](Span<const MValue> args, size_t nargout, Span<MValue> outs) {
                                ...
                                os << args[0].toScalar();
                                { outs[0] = MValue::fromString(os.str(), alloc); return; }
                            });
    ...
    engine.registerFunction("str2num",
                            [&engine](Span<const MValue> args, size_t nargout, Span<MValue> outs) {
                                auto *alloc = &engine.allocator();
                                try {
                                    { outs[0] = MValue::scalar(std::stod(args[0].toString()), alloc); return; }
                                } catch (...) {
    ...
    engine.registerFunction("str2double",
                            [&engine](Span<const MValue> args, size_t nargout, Span<MValue> outs) {
                                ...
                                try {
                                    ...
                                    { outs[0] = MValue::scalar(std::numeric_limits<double>::quiet_NaN(), alloc); return; }
                                }
                            });
    ...
    engine.registerFunction("strcmp",
                            [&engine](Span<const MValue> args, size_t nargout, Span<MValue> outs) {
                                auto *alloc = &engine.allocator();
                                { outs[0] = MValue::logicalScalar(args[0].toString() == args[1].toString(), alloc); return; }
                            });
    ...
    engine.registerFunction("strcmpi",
                            [&engine](Span<const MValue> args, size_t nargout, Span<MValue> outs) {
                                ...
                                { outs[0] = MValue::logicalScalar(a == b, alloc); return; }
                            });
    ...
    engine.registerFunction("upper",
                            [&engine](Span<const MValue> args, size_t nargout, Span<MValue> outs) {
                                ...
                                { outs[0] = MValue::fromString(s, alloc); return; }
                            });
    ...
    engine.registerFunction("strtrim",
                            [&engine](Span<const MValue> args, size_t nargout, Span<MValue> outs) {
                                ...
                                { outs[0] = MValue::fromString(s.substr(start, end - start + 1), alloc); return; }
                            });
