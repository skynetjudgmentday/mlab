...
void PlotLibrary::install(Engine &engine)
{
    ...
    auto parsePlotArgs = [](const std::vector<MValue> &args, size_t startIdx, DatasetInfo &ds) {
        for (size_t i = startIdx; i + 1 < args.size(); i += 2) {
            ...
            if (key == "linewidth")        ds.lineWidth = args[i + 1].toScalar();
            else if (key == "markersize")  ds.markerSize = args[i + 1].toScalar();
        }
    };

    // Helper: parse x/y/style arguments common to plot-like functions
    auto parsePlotXYStyle = [&vecToJson, &makeIndexJson](
        const std::vector<MValue> &args, DatasetInfo &ds) -> size_t {
        size_t nvStart = 2;
    ...
    auto registerLogPlot = [&](const std::string &name, const std::string &xscale,
    ...
    engine.registerFunction("grid",
                            [&engine](const std::vector<MValue> &args, size_t /*nargout*/) -> std::vector<MValue> {
                                ...
                                if (args.empty()) {
                                    ...
                                    if (arg == "on")         ax.gridMode = "on";
                                    else if (arg == "off")   ax.gridMode = "";
                                    else if (arg == "minor") ax.gridMode = (ax.gridMode == "minor") ? "on" : "minor";
                                }
                                fm.current().modified = true;
                                fm.emitModified();
                                return {MValue::empty()};
