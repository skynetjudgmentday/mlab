...
void DspLibrary::registerFilterFunctions(Engine &engine)
{
    ...
    engine.registerFunction(
        "filter", [](Span<const MValue> args, size_t nargout, Span<MValue> outs, CallContext &ctx) {
            ...
            for (size_t n = 0; n < nx; ++n) {
                ...
                for (size_t i = 1; i < nfilt; ++i) {
                    z[i - 1] = (i < nb ? bn[i] : 0.0) * x[n] - (i < na ? an[i] : 0.0) * y[n]
                               + (i < nfilt - 1 ? z[i] : 0.0);
                }
            }
            {
                ...
                for (size_t n = 0; n < len; ++n) {
                    ...
                    for (size_t i = 1; i < nfilt; ++i) {
                        z[i - 1] = (i < nb ? bn[i] : 0.0) * input[n]
                                   - (i < na ? an[i] : 0.0) * out[n] + (i < nfilt - 1 ? z[i] : 0.0);
                    }
                }
                return out;
            };
